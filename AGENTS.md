...
- **DB → git JSON:** Map publish writes `public.vhf_features`, then `website/sync-git.php` dispatches GitHub so Actions rewrites only `data/{CC}.json`. Direct REST/SQL upserts still rely on the 15-minute backup unless you run `node scripts/sync-vhf-to-git.js --country CC`. Live `sync-git.token` is a GitHub PAT; SFTP it next to `map.html` and never commit it.
