...
The website is already pointed at this project in `supabase-config.js` (anon public key only).
...
dispatch workflows on `htool/VHFinfoSite` and push to `htool/vhfinfo`). Put
**only** the token in a file named `sync-git.token` in the live site docroot
(same folder as `map.html`). Do not commit it. `.htaccess` blocks HTTP reads
of that file. The same PAT can be stored on this GitHub repo as
`VHFINFO_DATA_PUSH_TOKEN` so the Action can push data commits.
