...
const db = require("../features-db.js")

const ROOT = path.resolve(__dirname, "..")
const SKIP_FILES = new Set(["countries.json", "countries_bbox.json"])

function resolveDataDir() {
  if (process.env.VHFINFO_DATA_DIR) {
    return path.resolve(process.env.VHFINFO_DATA_DIR)
  }
  const sibling = path.resolve(ROOT, "..", "vhfinfo", "data")
  const nested = path.join(ROOT, "vhfinfo-data", "data")
...
function existingCountryFiles() {
  if (!fs.existsSync(DATA_DIR)) {
    throw new Error(
      "VHF data dir missing: " +
        DATA_DIR +
        " (set VHFINFO_DATA_DIR or clone htool/vhfinfo next to this repo)"
    )
  }
  return fs
